// Oil-field TERRITORY CONTROL — the objective that ties the game together.
// Control nodes are derived DETERMINISTICALLY from the terrain's oil-field
// richness peaks (pure, seed-based) so every client agrees on where they are
// with zero extra state. The server resolves who controls each node from live
// player presence, accrues score over time, and ends the round at a target
// score. PURE + transport-agnostic; the GameServer owns the mutable scoreboard.
...
export interface NodeStatus {
  ...
  controller: string;     // '' = uncontrolled / contested
  contested: boolean;
}
...
export interface PresencePlayer { name: string; x: number; z: number; dead: boolean; }

/**
 * Resolve who controls a node from live presence: count living players of each
 * name within the radius. A node is controlled iff exactly one name is present
 * (uncontested); two or more distinct names present -> contested (no score).
 */
export function resolveNode(node: ControlNode, players: PresencePlayer[]): NodeStatus {
  const present = new Set<string>();
  const r2 = node.radius * node.radius;
  for (const p of players) {
    if (p.dead || !p.name) continue;
    const dx = p.x - node.x, dz = p.z - node.z;
    if (dx * dx + dz * dz <= r2) present.add(p.name);
  }
  if (present.size === 1) {
    return { id: node.id, x: node.x, z: node.z, radius: node.radius,
      controller: [...present][0], contested: false };
  }
  return { id: node.id, x: node.x, z: node.z, radius: node.radius,
    controller: '', contested: present.size > 1 };
}
